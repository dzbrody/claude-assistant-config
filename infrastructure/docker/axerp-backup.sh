...
#
# Retention handled by S3 Lifecycle on axina-openproject-files:
#   7d  STANDARD → 90d STANDARD_IA → 365d GLACIER  (same policy as OpenProject backups)

set -euo pipefail

SITE="erp.axinagroup.com"
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] === AXERP Backup Start ==="
...
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] === AXERP Backup Complete ==="
